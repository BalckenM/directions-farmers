...
import type { createKiddingEventSchema } from "../../validators/goat/goat.validator";


// Post-process kidding event rows
function mapKidding(
  raw: Awaited<ReturnType<typeof goatRepo.findKiddingEventById>>,
) {
  if (!raw) return null;
  return { ...raw };
}
...
export const goatKiddingService = {
  listKiddingEvents: async (farmOwnerId: string) => {
    ...
    return rows.map((r) => mapKidding(r as Parameters<typeof mapKidding>[0]));
  },
