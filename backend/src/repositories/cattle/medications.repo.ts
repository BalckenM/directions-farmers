...
import { cattleMedicationLogs } from "../../db/schema";
...
export const cattleMedicationsRepo = {
  ...
  findMedicationLogById: (farmOwnerId: string, id: string) =>
    db
      ...
      .where(
        and(
          eq(cattleMedicationLogs.farmOwnerId, farmOwnerId),
          eq(cattleMedicationLogs.id, id),
        ),
      )
      .then((r) => r[0] ?? null),

  createMedicationLog: (data: any) =>
    db.insert(cattleMedicationLogs).values(data),
};
