...
import { cattleVaccinations } from "../../db/schema";
...
export const cattleVaccinationsRepo = {
  listVaccinations: (farmOwnerId: string) =>
    db
      ...
      .orderBy(desc(cattleVaccinations.dueDate)),
  ...
  findVaccinationById: (farmOwnerId: string, id: string) =>
    db
      ...
      .where(
        and(
          eq(cattleVaccinations.farmOwnerId, farmOwnerId),
          eq(cattleVaccinations.id, id),
        ),
      )
      .then((r) => r[0] ?? null),
