...
import { cattleBreedingRecords } from "../../db/schema";
...
export const cattleBreedingRepo = {
  ...
  findBreedingRecordById: (farmOwnerId: string, id: string) =>
    db
      ...
      .where(
        and(
          eq(cattleBreedingRecords.farmOwnerId, farmOwnerId),
          eq(cattleBreedingRecords.id, id),
        ),
      )
      .then((r) => r[0] ?? null),

  createBreedingRecord: (data: any) =>
    db.insert(cattleBreedingRecords).values(data),
