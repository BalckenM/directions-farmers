...
import { cattleWeightRecords } from "../../db/schema";
...
export const cattleWeightRepo = {
  ...
  findWeightRecordById: (farmOwnerId: string, id: string) =>
    db
      ...
      .where(
        and(
          eq(cattleWeightRecords.farmOwnerId, farmOwnerId),
          eq(cattleWeightRecords.id, id),
        ),
      )
      .then((r) => r[0] ?? null),

  createWeightRecord: (data: any) =>
    db.insert(cattleWeightRecords).values(data),
