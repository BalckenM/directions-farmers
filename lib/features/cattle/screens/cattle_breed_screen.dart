...
class CattleBreedScreen extends ConsumerStatefulWidget {
  const CattleBreedScreen({super.key, required this.breed});

  final String breed;

  @override
  ConsumerState<CattleBreedScreen> createState() => _CattleBreedScreenState();
}

class _CattleBreedScreenState extends ConsumerState<CattleBreedScreen> {
  ...
  Widget build(BuildContext context) {
    final animalsAsync = ref.watch(cattleBreedAnimalsProvider(widget.breed));
  ...
  List<CattleAnimal> _applySorted(List<CattleAnimal> animals) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _VDivider(),
          _StatCell(label: 'Cows', value: '$cows'),
          _VDivider(),
          _StatCell(label: 'Bulls', value: '$bulls'),
          _VDivider(),
          _StatCell(label: 'In Calf', value: '$pregnant'),
          if (avgWeight != null) ...[
            _VDivider(),
            _StatCell(
...
class _AnimalCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      borderRadius: AppRadius.card,
      onTap: () => context.push(AppRoutes.cattleDetailPath(animal.id)),
      child: Container(
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: AppRadius.card,
        ...
        child: Row(
          children: [
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      if ((animal.bodyConditionScore ?? 3) < 2)
                        _Badge(
                          icon: Icons.warning_amber_rounded,
                          label:
                              'BCS ${animal.bodyConditionScore?.toStringAsFixed(1)}',
            ...
            const Icon(Icons.chevron_right,
                color: AppColors.cattleColor, size: 18),
          ],
        ),
      ),
    );
  ...
  Color _sexColor(String sex) {
    if (sex == 'bull' || sex == 'steer' || sex == 'calf_male') {
