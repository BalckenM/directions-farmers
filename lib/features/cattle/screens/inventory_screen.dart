...
class _CattleInventoryScreenState
    extends ConsumerState<CattleInventoryScreen>
    with SingleTickerProviderStateMixin {
  late final TabController _tabs;

  final List<_StockItem> _items = [
    // Medicines
    _StockItem(id: 'm1', name: 'Terramycin LA', category: 'medicine', quantity: 250, unit: 'mL', minStock: 100),
    _StockItem(id: 'm2', name: 'Ivomec (Ivermectin)', category: 'medicine', quantity: 50, unit: 'mL', minStock: 100),
    _StockItem(id: 'm3', name: 'Lumpy Skin Vaccine', category: 'medicine', quantity: 20, unit: 'doses', minStock: 30),
    _StockItem(id: 'm4', name: 'Anthrax Vaccine', category: 'medicine', quantity: 40, unit: 'doses', minStock: 20),
    _StockItem(id: 'm5', name: 'Dip Solution (Triatix)', category: 'medicine', quantity: 5, unit: 'L', minStock: 10),
    // Supplements
    _StockItem(id: 's1', name: 'Protein Lick', category: 'supplement', quantity: 200, unit: 'kg', minStock: 50),
    _StockItem(id: 's2', name: 'Mineral Lick', category: 'supplement', quantity: 35, unit: 'kg', minStock: 50),
    _StockItem(id: 's3', name: 'Maize Meal', category: 'supplement', quantity: 500, unit: 'kg', minStock: 100),
    _StockItem(id: 's4', name: 'Silage', category: 'supplement', quantity: 800, unit: 'kg', minStock: 200),
  ];
  ...
  void _showAddDialog(String category) {
    ...
    final unitCtrl =
        TextEditingController(text: category == 'medicine' ? 'mL' : 'kg');
              ...
              setState(() {
                _items.add(_StockItem(
                  ...
                  unit: unitCtrl.text.trim().isEmpty
                      ...
                      : unitCtrl.text.trim(),
                  minStock:
                      double.tryParse(minCtrl.text.trim()) ?? 10,
                ));
              });
              Navigator.pop(ctx);
            },
  ...
  Widget build(BuildContext context) {
    final medicines =
        _items.where((i) => i.category == 'medicine').toList();
    final supplements =
        _items.where((i) => i.category == 'supplement').toList();
    final lowCount = _items.where((i) => i.isLow).length;
    final currentCategory =
        _tabs.index == 0 ? 'medicine' : 'supplement';
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text(
                'Min: ${_fmt(item.minStock)} ${item.unit}'),
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
