...
class EditCattleScreen extends ConsumerStatefulWidget {
  const EditCattleScreen({super.key, required this.cattleId});
  final String cattleId;

  @override
  ConsumerState<EditCattleScreen> createState() => _EditCattleScreenState();
}

class _EditCattleScreenState extends ConsumerState<EditCattleScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return animalAsync.when(
      loading: () => FarmScaffold(
        appBar: const FarmAppBar(title: 'Edit Cattle'),
      ...
      data: (animal) {
        ...
        if (!_loaded) {
          ...
          _niisController.text = animal.niisEidNumber ?? '';
          _brandController.text = animal.brandNumber ?? '';
          _loaded = true;
        }
  ...
  void _save() {
    ...
    final edits = <String, dynamic>{
      ...
      if (_niisController.text.trim().isNotEmpty)
        'niisEidNumber': _niisController.text.trim(),
      if (_brandController.text.trim().isNotEmpty)
        'brandNumber': _brandController.text.trim(),
      if (_notesController.text.trim().isNotEmpty)
        'notes': _notesController.text.trim(),
    };

    ref.read(cattleEditProvider.notifier).applyEdit(widget.cattleId, edits);
