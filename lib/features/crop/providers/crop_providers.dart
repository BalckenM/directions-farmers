...
final cropCategoriesProvider =
    FutureProvider<List<CropCategory>>((ref) async {
  return ref.watch(cropRepositoryProvider).getCropCategories();
});

final cropsProvider =
    FutureProvider.family<List<Crop>, String?>((ref, categoryId) async {
...
final cropFieldsProvider =
    FutureProvider.family<List<CropField>, String?>((ref, farmId) async {
  return ref.watch(cropRepositoryProvider).getFields(farmId: farmId);
});

final cropFieldByIdProvider =
    FutureProvider.family<CropField?, String>((ref, id) async {
...
final seasonsProvider =
    FutureProvider.family<List<CropSeason>, String?>((ref, farmId) async {
...
final upcomingCalendarEventsProvider =
  ...
  final all =
      await ref.watch(cropRepositoryProvider).getCalendarEvents();
  final now = DateTime.now();
  return all
      .where((e) =>
          e.scheduledDate.isAfter(now) && !e.isCompleted)
      .take(10)
      .toList();
});
...
final cropTasksProvider =
    FutureProvider.family<List<CropTask>, String?>((ref, farmId) async {
...
final actionRequiredAlertsProvider =
    FutureProvider<List<WeatherAlert>>((ref) async {
...
final sprayRecordsProvider =
    FutureProvider.family<List<SprayRecord>, String?>((ref, fieldId) async {
  return ref
      .watch(cropRepositoryProvider)
      .getSprayRecords(fieldId: fieldId);
});

// ── Expenses ──────────────────────────────────────────────────────────────────

final cropExpensesProvider =
    FutureProvider.family<List<CropExpense>, String?>((ref, farmId) async {
...
final cropSalesProvider =
    FutureProvider.family<List<CropSale>, String?>((ref, farmId) async {
...
final advisoryContentProvider =
    FutureProvider.family<List<AdvisoryContent>, String?>((ref, category) async {
  return ref
      .watch(cropRepositoryProvider)
      .getAdvisoryContent(category: category);
});

final latestAdvisoryProvider =
    FutureProvider<AdvisoryContent?>((ref) async {
  final all =
      await ref.watch(cropRepositoryProvider).getAdvisoryContent();
